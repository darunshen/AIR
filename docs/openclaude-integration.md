...
- 已支持通过只读 `workspace.ext4` + 可写 `workspace-upper.ext4` 在 guest 内形成 `/workspace`
- 已支持通过 `air session export-workspace <id> <output-dir>` 导出当前 merged workspace 结果
...
当前实现采用 host/guest relay，不直接暴露通用 guest 网络端口：

- guest 内 `127.0.0.1:18080` 接收普通 HTTP proxy / HTTPS CONNECT 流量
- guest 侧 `air-agent` 把该 TCP 流量转到 host vsock
- host 侧 AIR proxy 再连接外部 provider API
